...
class StatePlaces extends StatefulWidget {
  StatePlaces({Key? key, required this.idPark, required this.nbre_de_place})
      : super(key: key);
  final String idPark;
...
class _StatePlacesState extends State<StatePlaces> {
  ...
  Future<Map<String, bool>> initNumbers() async {
    dataplaces = await FirebaseFirestore.instance
        .collection("places")
        .doc(widget.idPark)
        .get();
    ...
    return Material(
      child: StreamBuilder<DocumentSnapshot<Map<String, dynamic>>>(
        ...
        builder: (context, snapshot) {
          while (snapshot.connectionState == ConnectionState.waiting || snapshot.hasError == true ) {
          ...
          return RollingSwitch.icon(
            ...
            onChanged: (bool state) async {
              ...
              await FirebaseFirestore.instance
                  .collection("places")
                  .doc(widget.idPark)
                  .update({"place_libre": count, "$i": state});
              setState(() {});
            },
