...
class _DetailsState extends State<Details> {
  ...
  void getReservationPlace() async {
    await FirebaseFirestore.instance
        ...
        .where("park", isEqualTo: "/parking/" + widget.id_park)
        .snapshots()
        .listen((event) {
                  ...
                  builder: (context, snapshot) {
                    ...
                    for (int i = 0; i < int.parse(widget.nombre_place); i++) {
                      if (document?.data()!["$i"] == false) {
                        if (placelibre.contains("$i") == false &&
                            listPlaceReservation.contains("$i") == false) {
                          placelibre.add("$i");
                          print("$placelibre");
                        }
                      }
